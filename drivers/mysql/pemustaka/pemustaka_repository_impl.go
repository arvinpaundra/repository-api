...
func (repository PemustakaRepositoryImpl) FindAll(ctx context.Context, query request.PemustakaRequestQuery, limit int, offset int) ([]domain.Pemustaka, int, error) {
	...
	err = repository.conn.WithContext(ctx).Model(&domain.Pemustaka{}).
		Where("fullname LIKE ? AND role_id LIKE ? AND departement_id LIKE ? AND study_program_id LIKE ? AND is_collected_final_project LIKE ? AND year_gen LIKE ?", "%"+query.Keyword+"%", "%"+query.RoleId+"%", "%"+query.DepartementId+"%", "%"+query.StudyProgramId+"%", "%"+query.IsCollectedFinalProject+"%", "%"+query.YearGen+"%").
